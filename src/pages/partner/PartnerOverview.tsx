  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Earned" value={`$${partnerStats.totalEarned.toLocaleString()}`} change={`${partnerStats.earningGrowth}% growth`} icon={DollarSign} delay={0} />
          <StatCard title="Referrals" value={partnerStats.totalReferrals.toString()} change="This month" icon={Users} delay={0.1} />
          <StatCard title="Active Content" value={partnerStats.activeContent.toString()} icon={BookOpen} delay={0.2} />
          <StatCard title="Revenue Share" value={`${partnerStats.revenueShare}%`} icon={Percent} delay={0.3} changeType="neutral" />
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="stat-card">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={partnerRevenue}>
              ...
              <Area type="monotone" dataKey="earned" stroke="hsl(162,63%,41%)" fill="url(#partnerGrad)" strokeWidth={2} />
            </AreaChart>
          </ResponsiveContainer>
        </motion.div>
      </div>
