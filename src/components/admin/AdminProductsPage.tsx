...
export default function AdminProductsPage() {
  ...
  if (productsQuery.isLoading || categoriesLoading) return <p>Loading...</p>;
  if (productsQuery.error) return <p>Error: {productsQuery.error.message}</p>;

  const handleAdd = () => {
    const priceNum = Number(form.price);
    if (!form.name || priceNum <= 0 || !form.categoryId) return;

    createProduct.mutate({ name: form.name, price: priceNum, categoryId: form.categoryId });
    setForm({ name: "", price: "", categoryId: "" });
  };

  const handleUpdate = (id: string) => {
    const priceNum = Number(editForm.price);
    if (!editForm.name || priceNum <= 0 || !editForm.categoryId) return;

    updateProduct.mutate({ id, payload: { name: editForm.name, price: priceNum, categoryId: editForm.categoryId } });
    setEditingId(null);
